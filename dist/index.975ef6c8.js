...
const generateCircle = (timer)=>{
    ...
    circleEl.style.top = window.innerHeight * 0.05 + Math.random(window.innerHeight) * 0.7 * window.innerHeight + "px";
    circleEl.style.left = window.innerWidth * 0.05 + Math.random(window.innerWidth) * 0.7 * window.innerWidth + "px";
...
const gameOver = (record)=>{
    ...
    if (localStorage.getItem("NewRecord") == 0) localStorage.setItem("NewRecord", record);
    if (localStorage.getItem("NewRecord") < record) localStorage.setItem("NewRecord", record);
...
const handleCircleClick = ()=>{
    ...
    scoreEl.innerText = scorePoints;
    if (scorePoints >= 100 && scorePoints < 200) circleEl.style.border = " 10px double rgb(255, 255, 255)";
    else if (scorePoints >= 200 && scorePoints < 300) circleEl.style.border = " 10px dotted rgb(255, 255, 255)";
    else if (scorePoints >= 300) circleEl.style.border = " 8px dashed rgb(255, 255, 255)";
};
const generateEvent = ()=>{
    ...
    circleEvent.style.top = Math.random(window.innerHeight) * 0.5 * window.innerHeight + "px";
    circleEvent.style.left = Math.random(window.innerWidth) * 0.5 * window.innerWidth + "px";
...
const setDarkMode = ()=>{
    ...
    document.querySelector("body").style.backgroundImage = "url(images/darkModeBg.jpg)";
    document.querySelector("body").style.backgroundSize = "37px";
    document.querySelector("body").style.animation = "bg-scrolling 0.4s infinite linear";
...
const setLightMode = ()=>{
    ...
    document.querySelector("body").style.animation = "bg-scrolling 0.32s infinite linear";
...
const setColorMode = ()=>{
    if (localStorage.getItem("mode") === "dark") setDarkMode();
    else setLightMode();
};
circleEl.addEventListener("mousedown", ()=>{
    handleCircleClick();
    const RNGvalue = Math.random(1);
    if (RNGvalue < 0.04) generateEvent();
});
// OPTIONS
...
setColorMode(); // window.addEventListener("mousedown", (e) => {
 // 	console.log(e);
 // });

},{}]},["igKGj","8lqZg"], "8lqZg", "parcelRequire94c2")
